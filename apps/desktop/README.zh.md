...
正式安装版会在启动后不久检查公开的 GitHub Releases，并在运行期间每六小时检查一次。发现新的稳定版本后，应用会在后台下载；悬浮更新控件显示下载进度，并在校验后的安装程序准备就绪时变成重启操作。该控件也可随时手动检查更新。选择重启时，应用会先停止内置后端，再由 Electron 安装更新并重新启动；选择稍后时，已下载的更新会保留到下次退出。开发版和免安装构建不会访问发行源。
