...
正式安装版会在启动后不久检查公开的 GitHub Releases，并在运行期间每六小时检查一次。发现新的稳定版本后，应用会在后台下载；蓝色更新控件只在存在更新时出现，显示下载进度，并在校验后的安装程序准备就绪时变成立即更新操作。点击后不再二次确认，应用会直接停止内置后端、安装更新并重新启动。如果没有点击，已下载的版本会在应用退出时安装，或者在下次启动识别到同一待安装版本后自动安装。开发版和免安装构建不会访问发行源。
