...
正式安装版会在启动后不久检查公开的 GitHub Releases，并在运行期间每六小时检查一次。发现新的稳定版本后，应用会在后台下载；蓝色更新控件只在存在更新时出现，显示下载进度，并在校验后的安装程序准备就绪时变成立即更新操作。下载完成不会安排应用退出时安装，也不会在以后启动时自动安装。只有用户点击准备就绪的更新操作后，应用才会停止内置后端、静默运行安装程序并重新启动，不再需要任何后续操作。开发版和免安装构建不会访问发行源。
