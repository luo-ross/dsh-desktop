...
describe('release families', () => {
  ...
  it('keeps the community desktop application outside the upstream npm release family', () => {
    const root = mkdtempSync(join(tmpdir(), 'dsh-release-community-'))
    roots.push(root)
    write(join(root, 'apps/cli/package.json'), '{"name":"@deepseek-ai/dsh","version":"0.0.1"}\n')
    write(join(root, 'apps/desktop-community/package.json'), JSON.stringify({
      name: '@luo-ross/dsh-desktop',
      version: '0.1.20',
      private: true,
    }))
