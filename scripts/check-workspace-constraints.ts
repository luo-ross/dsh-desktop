...
/**
 * Ordinary directories whose packages this repository publishes: one release
 * member each, except the application directories below.
 */
const standardReleaseMemberDirectory = /^(?:packages\/(?!experimental\/)[^/]+\/[^/]+|apps\/[^/]+|vendor\/[^/]+)$/
/** Installable applications assembled by electron-builder rather than published to npm. */
const desktopApplicationDirectories = new Set([
  'apps/desktop',
  'apps/desktop-host',
  'apps/desktop-community',
...
function isReleaseMemberDirectory(dir: string): boolean {
  return (standardReleaseMemberDirectory.test(dir) && !desktopApplicationDirectories.has(dir))
...
/**
 * Exact published versions a deployment package may depend on instead of a
 * workspace range: the desktop backend deploys the released CLI application
 * next to this repository's workspace packages.
 */
