...
describe('CI workflow', () => {
  ...
  it('keeps split native Windows PR jobs with failover, plus a master-only standby', () => {
    ...
    // The failover selectors are scoped to the upstream repository, so the
    // evaluation context must name it; outside it they resolve to hosted runners.
    ...
    // The Windows lanes must reach a hosted runner outside the upstream
    // repository, because the self-hosted label exists only there.
    expect(evaluateRunsOn(selectors.windows, {
      vars: { DSH_CI_FAILOVER_WINDOWS: 'selfhosted' },
      fromJSON: JSON.parse,
      github: { repository: 'luo-ross/dsh-desktop', event: { pull_request: { user: { login: 'maintainer' } } } },
    })).toBe('windows-2025')
