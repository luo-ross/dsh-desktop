...
describe('DeepSeekOnboardingDialog', () => {
  ...
  it('loads a credential-only modal, inerts the product, and focuses the key', async () => {
    ...
    const apiLink = screen.getByRole<HTMLAnchorElement>('link', { name: en.onboardingApiLink })
    expect(apiLink.href).toBe('https://platform.deepseek.com/api_keys')
    expect(apiLink.target).toBe('_blank')
    expect(apiLink.rel).toBe('noopener noreferrer')
