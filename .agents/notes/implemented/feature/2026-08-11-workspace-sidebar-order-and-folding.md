...
Search is a header action while collapsed and expands across the title and trailing actions. An outside click collapses a query that is empty after trimming but retains a non-empty query; while the rail search gesture is still in flight the outside-click listener stays unmounted ([rail-search self-dismissal](../bug-fix/2026-08-18-rail-search-outside-click-self-dismissal.md)). Compact Workspace and Session rows, a 24px bottom fade, and the absence of per-Workspace Session counts preserve vertical space without removing navigation affordances. The one-action reveal-location context menu keeps the standard menu style shared with the row-action menus. A closed reveal-location menu renders no anchor wrapper, so Electron's desktop-only action cannot insert an empty line box between the Workspace and its first Session. The first Session touches its Workspace header vertically and every grouped Session moves one 22px tree step to the right; flat and search rows retain their hierarchy-free alignment.
...
- Opening a Workspace never shows more than five Sessions without an explicit **Show more** gesture, and closing it resets only that transient gesture. Grouped Session titles remain visibly nested under the Workspace header rather than aligning as peer rows.
