...
The default gate ([.github/workflows/ci.yml](../../../../.github/workflows/ci.yml)) is deliberately keyless: it carries no secret and runs for forks. `test:e2e` self-skips without a key (`describe.skipIf(!process.env.DEEPSEEK_API_KEY)`), so adding it there would report green without exercising the real suite. A separate optional secret-bearing workflow provides real-API coverage when its repository owns a CI key.

## Decision

A dedicated workflow, [.github/workflows/e2e.yml](../../../../.github/workflows/e2e.yml), separate from ci.yml, runs only `pnpm run test:e2e` against the external API using a repo secret on trusted events. Its presence check skips all setup and test steps with a notice when the optional secret is absent, as decided in [Optional repository secret for real-API e2e](2026-08-14-optional-real-api-e2e-secret.md). The keyless workflow remains separate so forkable quality gates and secret-consuming real-API tests keep different trigger and credential policies.
...
Internal inference cost is not the limiting constraint, so a configured workflow optimizes for coverage and signal. It runs every matching `*.e2e.ts` file on multiple triggers and every trusted PR, implementing the [docs/testing.md](../../../../docs/testing.md) with-key policy.

### Triggers: trusted events only

`workflow_dispatch` + `push` to `main`/`master` + nightly `schedule` (`17 0 * * *`, 08:17 Asia/Shanghai) + `pull_request`. When the repository secret is configured, push gives a post-merge signal, schedule catches external-API drift, dispatch is the manual escape hatch, and trusted pull requests get a pre-merge gate. That pre-merge signal deliberately accepts the larger key-exposure surface described under § Security.
...
The gate is a *clean-skip nicety*, not the secret's security boundary (see § Security — the boundary is GitHub's own fork-secret withholding under `pull_request`). Without the gate, forks still could not read the key; they would run the presence check and report the same optional-coverage notice as a trusted repository without the secret.

### Presence check: explicit optional-coverage signal

The first step maps the repository secret and emits only an `available` boolean. An empty key produces a `::notice::` annotation and every setup and test step skips explicitly; a present key runs the complete suite. The successful missing-key result means optional real-API coverage was unavailable, not that the suite passed, while the separate keyless CI workflow remains the required quality signal. Desktop users' runtime keys never enter GitHub Actions.
...
- **Step-scoped secret.** `DEEPSEEK_API_KEY` is set in the `env:` of only the presence check and e2e steps, never job-level — so checkout/setup-node/install never see it. A compromised install-time lifecycle script in a dependency cannot read a secret that isn't in its environment.
...
A second CI workflow offers an optional repository secret to maintain. When configured, the real-API suite gates merges (pre-merge on trusted PRs, post-merge on the main branch) and runs nightly, so a real break in the agent's interaction with the external API surfaces in CI rather than only in a developer's local run — at the cost of real API calls on every trusted PR and merge. Without the secret, the workflow reports an explicit notice and succeeds without claiming real-API coverage.
...
The scheduled trigger auto-disables after 60 days of repo inactivity (a GitHub behavior); push/PR/dispatch are backstops, and an active monorepo will not hit it. Configured runs assume runner egress to `https://api.deepseek.com` — GitHub-hosted `ubuntu-latest` has it; an egress-restricted self-hosted runner would need connectivity confirmed before relying on the nightly.
