...
一个与 ci.yml 分离的专用工作流 [.github/workflows/e2e.yml](../../../../.github/workflows/e2e.yml) 使用 repo secret 对外部 API 运行且仅运行 `pnpm run test:e2e`，并仅在可信事件上触发。根据[真实 API e2e 使用可选的仓库密钥](2026-08-14-optional-real-api-e2e-secret.md)的决策，存在性检查会在可选 secret 缺失时输出 notice，并跳过所有设置和测试步骤。无密钥工作流保持独立，使可 fork 的质量门禁与消费 secret 的真实 API 测试各自拥有不同的触发和凭证策略。
...
内部推理（inference）成本不是限制因素，因此已配置密钥的工作流针对覆盖面和信号优化。它会在多种触发条件和每个受信任 PR（Pull Request）上运行所有匹配的 `*.e2e.ts` 文件，以落实 [docs/testing.md](../../../../docs/testing.md) 的有密钥策略。
