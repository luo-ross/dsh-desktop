...
默认门禁（[.github/workflows/ci.yml](../../../../.github/workflows/ci.yml)）刻意无密钥：不携带 secret，可供 fork 运行。`test:e2e` 在无密钥时自动跳过（`describe.skipIf(!process.env.DEEPSEEK_API_KEY)`），因此将其加入该工作流只会报绿而不会真正执行真实套件。一个独立且可选携带 secret 的工作流可在仓库拥有 CI 密钥时提供真实 API 覆盖。

## 决策

一个与 ci.yml 分离的专用工作流 [.github/workflows/e2e.yml](../../../../.github/workflows/e2e.yml) 使用 repo secret 对外部 API 运行且仅运行 `pnpm run test:e2e`，并仅在可信事件上触发。根据[真实 API e2e 使用可选的仓库密钥](2026-08-14-optional-real-api-e2e-secret.zh.md)的决策，存在性检查会在可选 secret 缺失时输出 notice，并跳过所有设置和测试步骤。无密钥工作流保持独立，使可 fork 的质量门禁与消费 secret 的真实 API 测试各自拥有不同的触发和凭证策略。
...
内部推理（inference）成本不是限制因素，因此已配置密钥的工作流针对覆盖面和信号优化。它会在多种触发条件和每个受信任 PR（Pull Request）上运行所有匹配的 `*.e2e.ts` 文件，以落实 [docs/testing.md](../../../../docs/testing.zh.md) 的有密钥策略。

### 触发条件：仅限可信事件

`workflow_dispatch` + `push` 到 `main`/`master` + 每夜 `schedule`（`17 0 * * *`，即北京时间 08:17）+ `pull_request`。配置仓库 secret 后，push 提供合并后信号，schedule 捕捉外部 API 漂移，dispatch 是手动逃生通道，可信 PR 获得合并前门禁。该合并前信号有意接受 § 安全性中描述的更大密钥暴露面。
...
该门禁是一个*干净跳过的便利措施*，而非 secret 的安全边界（见 § 安全性——边界是 GitHub 自身在 `pull_request` 下对 fork 的 secret 扣留机制）。没有该门禁，fork 仍然无法读取密钥；只会运行存在性检查，并像没有 secret 的可信仓库一样报告可选覆盖 notice。

### 存在性检查：明确标记可选覆盖

首个步骤映射仓库 secret，并且只输出 `available` 布尔值。密钥为空时会产生 `::notice::` 注解，并明确跳过所有设置和测试步骤；密钥存在时会运行完整套件。无密钥时的成功结果表示可选的真实 API 覆盖不可用，而不是套件已经通过；独立的无密钥 CI 工作流仍是必需的质量信号。桌面用户的运行时密钥绝不会进入 GitHub Actions。
...
- **步骤级 secret。** `DEEPSEEK_API_KEY` 仅在存在性检查和 e2e 步骤的 `env:` 中设置，从不在 job 级设置——因此 checkout/setup-node/install 永远看不到它。依赖中被入侵的安装时生命周期脚本无法读取不在其环境中的 secret。
...
- **不回显 secret。** 存在性检查只打印密钥是否已配置——不打印值或长度。

### 范围与运行时形态

配置可选的仓库 secret 后，job 仅在 Node 24 上运行 `test:e2e`；无密钥门禁和版本兼容性属于主 CI 工作流。测试通过 workspace paths 映射以未构建形式运行，使用有界的可配置 worker 池、逐测试重试和 job 超时。[被取代 CI 的取消策略](../process/2026-09-09-cancel-superseded-ci.zh.md)会在 PR、push、schedule 和手动触发之间取消同一工作流/引用组内的旧运行；合并后或每夜触发并不保证完成。
...
新增一个 CI 工作流，并提供一个可选维护的仓库 secret。配置后，真实 API 套件作为合并门禁（可信 PR 上的合并前门禁、主分支上的合并后门禁）并每夜运行，因此 agent 与外部 API 交互中的真实故障会在 CI 中浮现，而非仅在开发者的本地运行中出现——代价是每个可信 PR 和合并都会产生真实 API 调用。未配置 secret 时，工作流会输出明确的 notice，并在不声称拥有真实 API 覆盖的情况下成功结束。
...
schedule 触发器在仓库不活跃 60 天后会自动禁用（GitHub 行为）；push/PR/dispatch 是后备，活跃的 monorepo 不会触及此限制。已配置的运行假设 runner 对 `https://api.deepseek.com` 有出站连通性——GitHub 托管的 `ubuntu-latest` 具备此条件；受出站限制的自托管 runner 需要在依赖每夜运行之前确认连通性。
